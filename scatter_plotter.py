...
def _load_dataframe(uploaded_file):
    """Return a dataframe from a Streamlit UploadedFile, or None if parsing fails."""

    readers = (
        lambda file: pd.read_csv(file, encoding_errors="ignore"),
        pd.read_csv,
        pd.read_excel,
        lambda file: pd.read_excel(file, engine="openpyxl"),
    )
